...
app.get('/api/kids', auth, async (req,res)=>{
  const db = getPool();
  const [rows] = await db.query('SELECT id,name,grade,access_code FROM kids WHERE parent_id=? ORDER BY created_at DESC',[req.user.id]);
  res.json(rows);
});
...
app.post('/api/enroll', auth, async (req,res)=>{
  ...
  const [[kid]] = await db.query('SELECT id FROM kids WHERE id=? AND parent_id=?',[kid_id, req.user.id]);
  if(!kid) return res.status(403).json({error:'Not your kid'});
  await db.query('INSERT IGNORE INTO enrollments (competition_id,kid_id) VALUES (?,?)',[competition_id,kid_id]);
  res.json({ ok:true });
});
...
app.post('/api/attempts/start', auth, async (req,res)=>{
  ...
  const [[comp]] = await db.query('SELECT * FROM competitions WHERE id=? AND active=1',[competition_id]);
  ...
  const [[{cnt}]] = await db.query('SELECT COUNT(*) AS cnt FROM attempts WHERE competition_id=? AND kid_id=?',[competition_id,kid_id]);
  ...
  let questions;
  const [curated] = await db.query('SELECT q.* FROM comp_questions cq JOIN questions q ON q.id=cq.question_id WHERE cq.competition_id=? ORDER BY cq.ord ASC',[competition_id]);
  if(curated.length){
    questions = curated;
  }else{
    const [poolRows] = await db.query('SELECT * FROM questions WHERE subject=? ORDER BY RAND() LIMIT ?', [comp.subject, comp.questions_count]);
    questions = poolRows;
  }
...
app.post('/api/attempts/next', async (req,res)=>{
  ...
  const [[att]] = await db.query('SELECT * FROM attempts WHERE id=? AND token=?',[attempt_id, token]);
  ...
  let choices = [];
  if(q.type==='mcq'){
    const [rows] = await db.query('SELECT id,label FROM choices WHERE question_id=? ORDER BY RAND()', [qid]); // don’t leak correctness
    choices = rows;
  }
...
app.post('/api/attempts/answer', async (req,res)=>{
  ...
  if(q.type === 'mcq'){
    const [[opt]] = await db.query('SELECT is_correct FROM choices WHERE id=? AND question_id=?',[choice_id, question_id]);
    if(opt?.is_correct) { is_correct = 1; points = q.points; }
  } else {
    // Simple numeric compare (tweak as needed)
    const answer = Number(numeric_answer);
    const [[truth]] = await db.query('SELECT label FROM choices WHERE question_id=? AND is_correct=1 LIMIT 1',[question_id]);
  ...
  const [[{ sumPoints }]] = await db.query('SELECT COALESCE(SUM(points_awarded),0) AS sumPoints FROM attempt_answers WHERE attempt_id=?',[attempt_id]);
...
app.post('/api/attempts/finish', async (req,res)=>{
  ...
  await db.query('UPDATE attempts SET status=?, finished_at=NOW() WHERE id=?',[status, attempt_id]);
...
app.get('/api/admin/parents', auth, adminOnly, async (req, res) => {
  ...
  const [rows] = await db.query(sql, params); // use query(), not execute()
  res.json(rows);
});
...
app.get('/api/questions/:id', auth, adminOnly, async (req, res) => {
  const db = getPool();
  const id = Number(req.params.id);
  const [[qRow]] = await db.execute('SELECT * FROM questions WHERE id=?', [id]);
  if (!qRow) return res.status(404).json({ error: 'Not found' });
  const [choices] = await db.execute('SELECT id, label, is_correct FROM choices WHERE question_id=? ORDER BY id ASC', [id]);
  res.json({ ...qRow, choices });
});
